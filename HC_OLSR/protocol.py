...
def transmit_service_message(send_node, receive_node, accumulated_delay, accumulated_cost, message_size,
    ...
    # 计算延迟
    delay = calculate_mac_delay(next_hop, send_node, message_size * 8, 'hc_olsr')
...
def run_hc_olsr_protocol(node_count, simulation_steps, max_velocity, senders, receivers, send_times,
                        packet_sizes, initial_energy, position_file_path):
    """运行HC_OLSR协议仿真（混合OLSR：RF控制 + FSO业务）"""
    alive_history = []
    success_counts = []
    delay_history = []
    failed_stats = [0] * 5
    ...
    for current_time in tqdm(range(1, simulation_steps + 1), desc="HC_OLSR\t"):
        # 使用预计算的距离矩阵
    ...
    # 计算TC消息成功率
    tc_success_rate = average([average(node.tc_success) for node in nodes if node.tc_success]) / node_count
