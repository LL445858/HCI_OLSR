...
from BASE_Improve.node import Node
...
def initialize_nodes(node_count, initial_energy, positions, predictions):
    ...
    for i in range(node_count):
        ...
        # 设置初始预测位置
        nodes[i].position_predict = predictions[1, i, :].tolist() if predictions.shape[0] > 1 else position
    
    return nodes


def update_node_states(nodes, current_time, positions, predictions, simulation_steps):
    ...
    for i in range(node_count):
        if nodes[i].alive and current_time < simulation_steps - 1:
            nodes[i].position_predict = predictions[current_time + 1, i, :].tolist()
...
def update_routing_tables(nodes, distance_matrix, prediction_matrix):
    """更新所有节点的路由表"""
    for node in nodes:
        if node.alive:
            node.update_route(distance_matrix, prediction_matrix)
    return nodes
...
def transmit_service_message(sender, receiver, accumulated_delay, accumulated_cost, message_size, 
    ...
    delay = calculate_mac_delay(next_hop, sender, message_size * 8, 'hci_base')
...
def run_i_base_protocol(node_count, simulation_steps, max_velocity, senders, receivers, send_times,
                       packet_sizes, initial_energy, position_file_path, prediction_file_path):
    """运行I_BASE协议仿真（带位置预测的改进基础协议）"""
    prediction_file_path = prediction_file_path.replace('.txt', '_mmn.txt')
    ...
    for t in range(simulation_steps + 1):
        ...
        pred_matrix = calculate_distance_matrix(predictions[t])
        distance_matrices.append(dist_matrix)
        prediction_matrices.append(pred_matrix)
    
    for current_time in tqdm(range(1, simulation_steps + 1), desc="BASE_Improve\t"):
        # 使用预计算的距离矩阵
        current_distance_matrix = distance_matrices[current_time]
        current_prediction_matrix = prediction_matrices[min(current_time + 1, simulation_steps)]
        
        nodes = update_node_states(nodes, current_time, positions, predictions, simulation_steps)
        nodes = broadcast_hello_messages(nodes, current_distance_matrix)
        nodes = update_routing_tables(nodes, current_distance_matrix, current_prediction_matrix)
